...
export default {
  ...
  mutations: {
    [MUTATIONS.ADD_TO_INVENTORY]: (state, item) => {
      const key = item.id
      const inventoryItem = state.inventoryItems.find(item => item.id === key);
      if(inventoryItem) {
        inventoryItem.count += item.count
        return
      }
      const empty = state.inventoryItems.find(item => item.id.length === 0);
      if (empty) {
                empty.id = key
    ...
    [MUTATIONS.USE_ITEM]: (state) => {
      const item = state.inventoryItems[state.inventorySelectedIndex];
