...
export default {
  ...
  getters: {
    ...
    hasValidMoves: (state) => {
      ...
      const nonEmpty = state.flasks.map((f, i) => ({ f, i })).filter(({ f }) => f.length > 0)
      ...
      return nonEmpty.some(({ f: from, i: fromIdx }) => {
        ...
        return state.flasks
          ...
          .some(
            ({ t }) => t.length < MAX_LAYERS && (t.length === 0 || t[t.length - 1] === topColor)
          )
      })
    },
    timerDisplay: (state) => {
  ...
  actions: {
    ...
    pour({ state, commit }, { fromIdx, toIdx }) {
      ...
      const isWin = newFlasks
        .filter((f) => f.length > 0)
        .every((f) => new Set(f).size === 1 && f.length === MAX_LAYERS)
